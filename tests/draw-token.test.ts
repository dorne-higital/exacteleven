...
describe('draw-token', () => {
    it('round-trips: a token verifies back to the player id it was signed for, in the game it was signed for', async () => {
        ...
        await expect(verifyPlayerToken(token, GAME_ID, SECRET)).resolves.toBe('player-1');
    });

    it('rejects a token presented against a different game id than it was signed for', async () => {
        const token = await signPlayerToken('player-1', GAME_ID, SECRET);

        await expect(verifyPlayerToken(token, 'some-other-game', SECRET)).resolves.toBeNull();
    });

    it('rejects a token signed with a different secret', async () => {
        const token = await signPlayerToken('player-1', GAME_ID, SECRET);

        await expect(verifyPlayerToken(token, GAME_ID, 'wrong-secret')).resolves.toBeNull();
    });
    ...
    it('rejects a tampered token whose payload was swapped for a different player id', async () => {
        ...
        await expect(verifyPlayerToken(tampered, GAME_ID, SECRET)).resolves.toBeNull();
    });

    it('rejects a token with no separator', async () => {
        await expect(verifyPlayerToken('not-a-real-token', GAME_ID, SECRET)).resolves.toBeNull();
    });

    it('never throws on garbage input, including an empty string', async () => {
        await expect(verifyPlayerToken('', GAME_ID, SECRET)).resolves.toBeNull();
        await expect(verifyPlayerToken('....', GAME_ID, SECRET)).resolves.toBeNull();
        await expect(verifyPlayerToken('garbage.garbage', GAME_ID, SECRET)).resolves.toBeNull();
    });

    it('rejects a token older than the 5-minute expiry window', async () => {
        ...
        try {
            ...
            await expect(verifyPlayerToken(token, GAME_ID, SECRET)).resolves.toBeNull();
        } finally {
            Date.now = realNow;
        }
    });
